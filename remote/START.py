...
# HOST = '192.168.0.96'    # Server(Raspberry Pi) IP address
HOST = raw_input('Watney IP Address: ') # User input for IP address
PORT = 21567
BUFSIZ = 1024             # buffer size
ADDR = (HOST, PORT)

top = Tk()   # Create a top window
top.title('Watney Control Center')
top.focus_set()
...
top.bind('<KeyPress-d>', x_increase)
top.bind('<KeyPress-a>', x_decrease)
top.bind('<KeyPress-s>', y_decrease)
top.bind('<KeyPress-w>', y_increase)
top.bind('<KeyPress-f>', xy_home)
...
# =============================================================================
top.bind('<KeyPress-Left>', left_fun)   # Press down key 'A' on the keyboard and the car will turn left.
top.bind('<KeyPress-Right>', right_fun) 
top.bind('<KeyPress-Down>', backward_fun)
top.bind('<KeyPress-Up>', forward_fun)
top.bind('<KeyPress-h>', home_fun)
top.bind('<KeyRelease-Left>', home_fun) # Release key 'A' and the car will turn back.
top.bind('<KeyRelease-Right>', home_fun)
top.bind('<KeyRelease-Down>', stop_fun)
top.bind('<KeyRelease-Up>', stop_fun)
